...
#include "HashUser.h"		// HashTable containing Users indexed by name
#include "HashInterest.h"	// HashTable containing Users indexed by interest
#include "HashLogin.h"		// HashTable containing Users indexed by login
#include "FileIO.h"			// Handles File Input & Output
#include "User.h"			// Our Abstract Data

using namespace std;

void debugDataStructures(BST<User> userBST, HashUser hashUsers, HashLogin hashLogin, HashInterest hashInterests, Graph userNetwork){
	...
	cout << "\n------------------ userNetwork ----------------\n";
	userNetwork.printGraph(cout);

	cout << endl << endl;
}

// Unique Function: Formats two strings so first letter capitalize and rest are not
...
// Unique Function: Formats a single string so each word is capitalized and rest are not, and removes all spaces in string
...
void myFriendRecs(User u, Graph& g){
/*
	List<Graph::friendRec> rankedUsers;
	g.getFriendRecs(u, rankedUsers);

	cout << "\nYour recommended friends: \n";
	rankedUsers.startIterator();
	int i = 1;
	while(!rankedUsers.offEnd()){
		Graph::friendRec temp = rankedUsers.getIterator();
		cout << i << ": " << temp.newfriend.getFirstName() << " " << temp.newfriend.getLastName()
		<< " has " << temp.numMutualFriends << " mutual friends and "
		<< temp.numMutualInterests << " mutual interests.\n";
		rankedUsers.advanceIterator();
		i++;
	}
	cout << "\nPlease enter the name of the friend you'd like to add: ";
	string f;
	//cin >> f;
	cout << endl << endl;
	*/
	List<User> recommendedFriends = g.getFriendRecommendations(u);

	// Prompt User to Add friend
	// We can now check if user input is within bounds (recommendedFriends.linearSearch())
}
...
void userAddFriend(User &sourceUser, User temp, BST<User> &userBST, HashUser &hashUsers, HashLogin &hashLogin){
	...
	// Graph - Insert User into Graph and call addEdge
...
void userRemoveFriend(User &sourceUser, User temp, BST<User> &userBST, HashUser &hashUsers, HashLogin &hashLogin){
	...
	// Graph - Insert User into Graph and call removeEdge
	userBST.insert(sourceUser);
	userBST.insert(temp);
	...
	while(true){
		...
		if(index != -1){
			...
			while(i == "Y" || i == "y"){
				...
				}else if(hashInterests.checkUserWithInterest(temp,fInterest) && source.searchFriends(temp) == false){
					//temp = hashUsers.getUser(name);
	...
	while(true){
		...
		if(!cin){
			...
			if(hashUsers.search(temp) != -1 && source.searchFriends(temp) == false){
				...
				hashUsers.searchResults(firstName,lastName,source);
...
int main(){
	...
	// Load User info from text file into data structures
	file.loadData("UserData.txt",userBST,hashUsers,hashLogin,hashInterests,userNetwork);

	/*
	userNetwork.printGraph(cout);
	userNetwork.BFS(12);
	userNetwork.printBFS(cout);
	cout << "Done" << endl;
	*/
	...
	*/
	...
	while(true){
		if(input == 4)
			loginScreen(hashLogin,sourceUser);	// Login Screen

		input = mainMenuPrompt(sourceUser);		// Home Screen
		...
		case 2: searchForNewFriends(sourceUser,userBST,hashUsers,hashLogin,hashInterests);	// Search For New Friends
				break;
		case 3: myFriendRecs(sourceUser, userNetwork);										// Get Friend Recommendations
				break;
		...
		case 5: file.saveData("OutputUserData.txt",userBST);								// Exit Program & Debug Status
				debugDataStructures(userBST,hashUsers,hashLogin,hashInterests,userNetwork);
